...
def _ensure_log_file(log_dir: str) -> None:
    """将 loguru 同时输出到文件，便于长时间运行后排查问题。"""
    global _LOG_FILE
    if _LOG_FILE is not None:
        return
    log_path = os.path.join(log_dir, "convert_to_md.log")
    _LOG_FILE = logger.add(
        log_path,
        rotation="20 MB",
        retention=5,
        level="DEBUG",
...
def do_parse(
    ...
    if backend == "pipeline":
        ...
        for idx, model_list in enumerate(infer_results):
            ...
            logger.info("Processing [{}/{}] {}", idx + 1, total, pdf_file_name)
            try:
                ...
                logger.info("Done [{}/{}] {}", idx + 1, total, pdf_file_name)
            except Exception as e:
                logger.error("Failed at file [{}]: {} -- {}", idx + 1, pdf_file_name, e)
                raise
    else:
...
def parse_doc(
    ...
    """
    _ensure_log_file(str(Path(output_dir).resolve().parent))
    total = len(path_list)
    logger.info("parse_doc started: output_dir={}, backend={}, total files={}", output_dir, backend, total)
    try:
        file_name_list = []
        pdf_bytes_list = []
        lang_list = []
        ...
        logger.info("parse_doc finished: all {} files done", total)
    except Exception as e:
        logger.exception("parse_doc failed: {} (see traceback above)", e)
        raise
...
if __name__ == '__main__':
    ...
    pdf_files_dir = os.path.join(__dir__, "downloads")
    output_dir = os.path.join(__dir__, "output")
    pdf_suffixes = ["pdf"]
    image_suffixes = ["png", "jpeg", "jp2", "webp", "gif", "bmp", "jpg"]

    doc_path_list = []
    for doc_path in Path(pdf_files_dir).rglob('*'):
        if guess_suffix_by_path(doc_path) in pdf_suffixes + image_suffixes:
            doc_path_list.append(doc_path)

    _ensure_log_file(__dir__)
    logger.info("Found {} documents under downloads/", len(doc_path_list))
